...
import { addApartment, findApartment, getAllActiveUsersList, updateApartment } from "../../services/endpoints";
...
function SingleApartment() {
  ...
  const handleSubmit = async () => {
    let response
    if (params.id !== 'new') {
      response = await updateApartment(params.id)
    } else {
      response = await addApartment()
    }
  ...
  useEffect(() => {
    if (params.id !== 'new') {
      async function fetchData() {
        ...
        setNo(response.data.data.phoneNumber)
          ...
          <div className="card">
            <div className="card-body">
              <form>
                ...
                <div className="form-group">
                  <label className="row">
                    <p className="col-sm-2 col-form-label">Is Active</p>
                    <div className="col-sm-10">
                      <select className="form-control" id="isActive" onChange={(e) => { setIsActive(e.target.value) }} value={isActive}>
